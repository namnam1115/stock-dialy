...
## 8. 改善ループ（アウトバウンドの harness）

§1〜7 は「AIに実装させる側（インバウンド）」の harness — 良いコンテキストを供給し、
壊れない構造にし、自分で検証させる。その一段上に、**改善サイクルそのものを誰が駆動するか**
（アウトバウンド）の harness を置く。改善の駆動を規約でなく**構造**に落とす：

- **バックログ（正本）** — 改善候補を1箇所に集約（原則2）。`docs/improvement_loop.md §3`。
- **方向性ゲート（人間）** — AIは提案まで自走し、承認前にコードを触らない。着手は承認項目のみ
  （これがループの唯一の不変条件・原則1）。
- **手動起点（`/improve`）** — セッション起点で1周回す。定期実行はしない（過剰自動化を避ける）。

信号源は3つに限定：①判断軸との差分（`product_context.md`）②UI/UXレビュー（`uiux_review.md`）
③使用感（ドッグフーディング・`run-app`）。実行後は §5 学習記録に1行残し、次の観測を賢くする（原則6）。
→ 正本: `docs/improvement_loop.md` / 起動: `.claude/commands/improve.md`
...
- 改善ループ（アウトバウンドの harness・バックログ＋ゲート）: `docs/improvement_loop.md`
