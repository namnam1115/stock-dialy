...
**v2（2026-07-11）：業種（sector）要素を追加。v1 では対象外としていたが、ユーザー要望により
`StockDiary.sector` を要素として組み込んだ（§2・§5 を参照）。**
...
- **ノードの種類**：`tag`（要素）／`sector`（要素＝業種）／`stock`（銘柄＝ユーザーの同一銘柄の日記群を1ノードに集約）。
- **隣接の定義**：
  - `tag` ノードの隣接 ＝ **子タグ**（`Tag.parent = self`）＋ **親タグ**（`self.parent`）＋
    **そのタグが付いた銘柄**（distinct symbol）
  - `sector` ノードの隣接 ＝ **その業種（`StockDiary.sector`）が設定された銘柄**（distinct symbol）
  - `stock` ノードの隣接 ＝ **その銘柄に付いた全タグ** ＋ **その銘柄の業種**（設定があれば1件）
- **操作**：ノードをタップ＝隣接を1段展開。もう一度タップ＝畳む（子孫も畳む）。
  展開していないノードは出さない。→ 画面には「今たどっている文脈」だけが残る。
- **v2 で「要素」= タグ + 業種に拡張**（v1 はタグのみ）。業種は `StockDiary.sector`
  （自由記述・フォームで手動/自動入力）をそのまま要素名として使う。CompanyMaster による
  未設定業種の補完（`get_sector_graph_data` が行うもの）はこのドリルダウンでは行わない
  （sector 未設定の日記はこの探索の入口に出ない＝シンプルさ優先）。
...
ノード識別子：
- タグ：`tag:<pk>`
- 業種：`sector:<name>`（`StockDiary.sector` の値そのもの）
...
**v2（2026-07-11 追加）**
- 業種（sector）要素のドリルダウン（業種→銘柄、銘柄→業種）。入口検索にも業種を追加。

**v2 時点でも非対象（将来）**
- @メンション・手動リンク等の他エッジ種、方向（追い風/向かい風）色分け、
  複数起点の同時探索、保存/共有。
